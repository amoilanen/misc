...
pub fn encrypt(data: &[u8], key: &[u8], key_size: KeySize, mode: Mode, iv: Option<&[u8]>) -> Vec<u8> {
...
pub fn decrypt(data: &[u8], key: &[u8], key_size: KeySize, mode: Mode, iv: Option<&[u8]>) -> Vec<u8> {
    let round_keys = key_expansion(key, key_size);
...
mod tests {
    ...
    fn test_encrypt_decrypt_ecb() {
        ...
        let ciphertext = encrypt(&plaintext, &key, KeySize::Bits128, Mode::ECB, None);
        let decrypted = decrypt(&ciphertext, &key, KeySize::Bits128, Mode::ECB, None);
    ...
    fn test_encrypt_decrypt_cbc() {
        ...
        let ciphertext = encrypt(&plaintext, &key, KeySize::Bits128, Mode::CBC, Some(&iv));
        let decrypted = decrypt(&ciphertext, &key, KeySize::Bits128, Mode::CBC, Some(&iv));
